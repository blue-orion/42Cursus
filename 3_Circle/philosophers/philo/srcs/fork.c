...
/*   Updated: 2024/12/21 22:13:57 by takwak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pick_up_left_fork(t_philo *philo)
{
	...
	if (pthread_mutex_lock(&philo->info->mutex[left_num]))
		perror("mutex_lock");
	philo->left_fork = 1;
	philo->info->fork[left_num] = 1;
	time = get_current_time(&philo->cur_time, &philo->info->start_time);
	print_log(philo->num, time, FORK, &philo->info->print_mutex);
	return (0);
}
...
int	pick_up_right_fork(t_philo *philo)
{
	...
	if (pthread_mutex_lock(&philo->info->mutex[right_num]))
		perror("mutex_lock");
	philo->right_fork = 1;
...
int	put_down_left_fork(t_philo *philo)
{
	...
	left_num = philo->num % philo->info->num_of_philo;
	if (pthread_mutex_unlock(&philo->info->mutex[left_num]))
		perror("mutex_unlock");
	philo->left_fork = 0;
...
int	put_down_right_fork(t_philo *philo)
{
	...
	right_num = philo->num - 1;
	if (pthread_mutex_unlock(&philo->info->mutex[right_num]))
		perror("mutex_unlock");
	philo->right_fork = 0;
