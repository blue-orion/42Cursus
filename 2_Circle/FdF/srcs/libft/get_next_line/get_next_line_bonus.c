...
/*   Updated: 2024/11/17 21:22:40 by takwak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	exist_line(char **res, t_buffer *info)
{
	...
	if (info->last)
	{
		...
		if (ft_strchr_gnl(start_adr, '\n') != NULL)
		{
			len = ft_strchr_gnl(start_adr, '\n') + 1 - start_adr;
...
char	*get_next_line(int fd)
{
	...
	res = NULL;
	if (info == NULL)
		info = ft_newlst_gnl(&info, info, fd);
	list = info;
	while (list != NULL && list->label != fd)
		list = list->next;
	if (list == NULL)
		list = ft_newlst_gnl(&info, list, fd);
